...
* **cropByFocalArea(targetX, targetY, desiredWidth, desiredHeight)** — crops the image based on a focal point on the original image and the desired output width and height. If the provided inputs would generate an image out of bounds of the original the focal point is "moved" so a image with expected dimensions is still returned. The focal point of the resulting image is always at its center.
